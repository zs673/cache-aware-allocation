...
def draw_box(x, NOS_NUM, PATH):
    ...
    boxplot_data = [x[0:TOT_NOS, :], x[TOT_NOS:2*TOT_NOS, :], x[2*TOT_NOS:3*TOT_NOS, :], x[3*TOT_NOS:4*TOT_NOS, :],x[4*TOT_NOS:, :]]
    ...
    legend_elements = [plt.Line2D([0], [0], color=color_list[0], label='WF+EO'),
                    ...
                    plt.Line2D([0], [0], color=color_list[2], label='New Allocation wo Node Deferment'),
                    plt.Line2D([0], [0], color=color_list[3], label='New Allocation'),
...
def draw_bar(x, NOS_NUM, PATH):
    ...
    A = x[2*TOT_NOS:3*TOT_NOS, :].mean(axis=0)
    B = x[3*TOT_NOS:4*TOT_NOS, :].mean(axis=0)
    ...
    plt.bar(x_list_box[2] , A, box_width, color='g', align='center', label='New Allocation wo Node Deferment')#柱宽为0.2，标签在柱子中间
    plt.bar(x_list_box[3] , B, box_width, color='y', align='center', label='New Allocation')
