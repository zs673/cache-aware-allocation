...
def draw_box(x, NOS_NUM, PATH):
    ...
    boxplot_data = [x[0:TOT_NOS, :], x[TOT_NOS:2*TOT_NOS, :], x[2*TOT_NOS:3*TOT_NOS, :], x[3*TOT_NOS:, :]]
    ...
    legend_elements = [plt.Line2D([0], [0], color=color_list[0], label='AJLR'),
                    plt.Line2D([0], [0], color=color_list[1], label='New Allocation wo Node Deferment'),
                    plt.Line2D([0], [0], color=color_list[2], label='New Allocation'),
                    plt.Line2D([0], [0], color=color_list[2], label='New Priority + New Allocation')]
...
def draw_bar(x, NOS_NUM, PATH):
    AJLR = x[0:TOT_NOS, :].mean(axis=0)
    A = x[TOT_NOS:2*TOT_NOS, :].mean(axis=0)
    B = x[2*TOT_NOS:3*TOT_NOS, :].mean(axis=0)
    C = x[3*TOT_NOS:, :].mean(axis=0)
    ...
    #绘制并列柱状图
    plt.bar(x_size, AJLR, bar_width, color='xkcd:grey', align='center', label='AJLR')#柱宽为0.2，标签在柱子中间
    plt.bar(x_size + bar_width, A, bar_width, color='b', align='center', label='New Allocation wo Node Deferment')  #柱宽为0.2
    plt.bar(x_size + 2 * bar_width , B, bar_width, color='y', align='center', label='New Allocation')#柱宽为0.2，标签在柱子中间
    plt.bar(x_size + 3 * bar_width , C, bar_width, color='r', align='center', label='New Priority + New Allocation')
...
def main():
    ...
    PATH1 = "E:/Code/Java/cache-aware-allocation-main/result/AJLR priority defer/"
