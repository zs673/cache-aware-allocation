...
def main():
    PATH1 = "E:/Code/Java/cache-aware-allocation-main/result/33/"
    PATH2 = "E:/Code/Java/cache-aware-allocation-main/result/34/"
    data = pd.read_table(PATH1 + '/makespan_4_2.0.txt', sep=',', header=None)
    # data = data.iloc[NOS_NUM:, 0:10]

    data1 = data.iloc[0:400, 0:TASK_NUM*INSTANCE_NUM]
    x1 = data1.values
    data2 = data.iloc[400:, 0:TASK_NUM*INSTANCE_NUM]
    x2 = data2.values

    data = pd.read_table(PATH2 + '/makespan_4_2.0.txt', sep=',', header=None)
    # data = data.iloc[NOS_NUM:, 0:10]
