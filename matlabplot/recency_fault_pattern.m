...
for path = path_results
   subplot(subfigure_count,1,count);
    ...
    line([20.5 20.5], [0 1.1],'LineStyle',':','color','k','LineWidth',1);
     ...
     for util = 1:5
       if(util == 5)
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_4.0' , '_TIME_DEFAULT' , '.txt'));
       else
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_',num2str(util*0.8) ,'_TIME_DEFAULT', '.txt'));
       end
    ...
    for util = 1:5
       if(util == 5)
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_4.0' , '_TIME_STEP' , '.txt'));
       else
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_',num2str(util*0.8) ,'_TIME_STEP', '.txt'));
       end
    ...
    for util = 1:5
       if(util == 5)
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_4.0' , '_TIME_CURVE' , '.txt'));
       else
           data = readmatrix(strcat(path, 'recency_pattern/',metric ,'1', '_',num2str(util*0.8) ,'_TIME_CURVE', '.txt'));
       end
...
saveas(gcf,strcat(path_figs, 'ep_recency_fault_pattern.png'));