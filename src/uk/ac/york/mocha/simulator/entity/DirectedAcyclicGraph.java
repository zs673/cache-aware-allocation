...
public class DirectedAcyclicGraph implements Serializable {
	...
	public void Rank_Up(List<Node> nodes_topology) {
		for (int i = 0; i < nodes_topology.size(); i++) {
	...
	public void findPath(boolean longest) {
		...
		/*
		 * compute a longest/shortest path from begin to end
		 */
		...
		for (Node n : flatNodes) {
			...
			// self changes
			if (maxDiff < n.degree_diff)
				maxDiff = n.degree_diff;
			if (maxUp < n.rank_up)
				maxUp = n.rank_up;
			if (maxDown < n.rank_down)
				maxDown = n.rank_down;
			if (maxOrder < n.topology_order)
				maxOrder = n.topology_order;
		}
		
		/*
		 * calculate the max length of local path containing n
		 */
		for (Node n : flatNodes) {
		...
		for (Node n : flatNodes) {
			...
			n.weights[6] = Double.parseDouble(df.format((double) n.degree_diff / (double) maxDiff));
			n.weights[7] = Double.parseDouble(df.format((double) n.rank_up / (double) maxUp));
			n.weights[8] = Double.parseDouble(df.format((double) n.rank_down / (double) maxDown));
			n.weights[9] = Double.parseDouble(df.format((double) n.topology_order / (double) maxOrder));
