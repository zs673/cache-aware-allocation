...
public class SystemGenerator {
	...
	private List<DirectedAcyclicGraph> generateSporadicDAGs(List<Long> periods, boolean hard) {
		...
		for (int i = 0; i < total_tasks; i++) {
			StructuralParameters dag_param = new StructuralParameters(SystemParameters.maxLayer,
					...
					SystemParameters.connectProb, ran);//加入随机因素的
	...
	/*
	 * generate scheduling parameters for DAGs  DAG level的调度参数，比如DAG的优先级、周期、WorkLoad(sum of WCET of nodes)等
	 */
	private List<SchedulingParameters> generateSchedParam(List<Long> periodsT) {
		/*
		 * generates uniformly distributed periods 周期
		 */
		List<Long> periods;
		if (periodsT == null) {
	...
	public List<Double> generateCoresSpeed(int cores, boolean fixed) {
