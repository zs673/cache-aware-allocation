...
public class CorrelationCoefficient_fixedDAG {
	...
	public static void start(int nop) {
		...
		for (int i = 0; i < allPercent.length; i++) {
			faults(allCores[0], allPercent[i], 1.0, allInstanceNum[0], nop);
		}
		...
		for (int i = 0; i < nop; i++) {
			...
			runners.add(new Thread(new Runnable() {
				...
				public void run() {
					...
					List<List<Pair<Double, Long>>> result = runOneThread(cores, percent, effect, instanceNum,
...
//			System.out.println();
	...
	public static List<List<Pair<Double, Long>>> runOneThread(int cores, double percent, double effect, int instanceNum,
...
//			results.addAll(run(sys, cores, seed, faultType.all_nodes, percent, effect, print));
	...
	public static List<Pair<Double, Long>> run(Pair<List<DirectedAcyclicGraph>, CacheHierarchy> sys, int cores,
			int seed, faultType type, double percent, double effect, boolean print) {
		...
		Pair<Double, Long> makespan = oneRun(sys, cores, seed, print);
	...
	public static List<List<Node>> setUpSpecificFaults(List<DirectedAcyclicGraph> dags, faultType type, double percent,
			double effect, boolean opposite, boolean print) {
		...
		for (DirectedAcyclicGraph d : dags) {
			...
			for (Node n : faultNodes) {
				n.hasFaults = true;
				n.cvp.median = 0;
				n.cvp.range = effect; // ((double) rng.nextInt(effect +
				// 1) / (double) 100) / 3.0;
				if (print)
	...
	public static Pair<Double, Long> oneRun(Pair<List<DirectedAcyclicGraph>, CacheHierarchy> sys, int cores, int seed,
			boolean print) {
