...
public class OnlineGYY extends AllocationMethods {
    ...
    public void allocate(List<DirectedAcyclicGraph> dags, List<Node> readyNodes, List<List<Node>> localRunqueue,
         ...
         * || (localRunqueue.get(i).size() > 0 && localRunqueue.get(i).get(0).start >
         * currentTime))
         * && procs[i] <= currentTime) {
         * freeProc.add(i);
         * }
        ...
        for (Node tmp : nodesTobedone) {
            for (Node child : tmp.getChildren()) {
                ...
                for (Node parent : child.getParent()) {
                    ...
                    if (nodesTobedone.contains(parent)) { //haven't been finished before and would be finished this turn
                        worst_time = Math.max(worst_time, parent.finishAt);//最快能运行的时间
                    }
                }
                if (isReady) {
    ...
    private boolean is_future_node(List<Node> readyNodes, List<Node> futureNodes) {
        ...
        return futureNodes.get(0).sensitivity >= readyNodes.get(0).sensitivity
                && futureNodes.get(0).rank_down >= readyNodes.get(0).rank_down;
    }
