...
public class OnlineCARVB extends AllocationMethods {
	...
	public void allocate(List<DirectedAcyclicGraph> dags, List<Node> readyNodes, List<List<Node>> localRunqueue,
		...
		readyNodes.stream().forEach(c -> c.partition = -1);// init the partition of each ready node

		readyNodes.sort((c1, c2) -> Utils.compareNodeByPriorityAndSensitivity(dags, c1, c2));// sort the ready nodes by
																								// the sensitivity

		// select the number of the nodes, the nodes size if the minumum value between
		// the idle cores and the whole nodes size
