...
public class OnlineWFDNewSimu_Base extends AllocationMethods {
	
	public static int based_order_counter = 0;
	...
	public void allocate(List<DirectedAcyclicGraph> dags, List<Node> readyNodes, List<List<Node>> localRunqueue,
		...
		for (int i = 0; i < availableCores.size(); i++) {
			...
			if(readyNodes.get(i).fixed_order < 0) {
				readyNodes.get(i).fixed_order = based_order_counter;
				readyNodes.get(i).fixed_allocation = readyNodes.get(i).partition;
