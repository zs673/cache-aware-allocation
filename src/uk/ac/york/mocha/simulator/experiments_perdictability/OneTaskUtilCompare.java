...
public class OneTaskUtilCompare {
	...
	public static void oneTaskWithFaults() {
		...
		for (int i = 0; i < 1000; i++) {
			SystemGenerator gen = new SystemGenerator(cores, 1, true, true, null, 1000 + i, true, SystemParameters.printGen);
			Pair<List<DirectedAcyclicGraph>, CacheHierarchy> sys = gen.generatedDAGInstancesInOneHP(intanceNum, hyperPeriodNum, null,
					false);
			allSys.add(sys);
		}
		...
		for (int i = 8; i <= 40; i = i + 8) {
			...
			RunOneGroup(1, intanceNum, hyperPeriodNum, true, null, seed, seed, null, 1000, true, ExpName.predict, allSys);
		}
	...
	public static void RunOneGroup(int taskNum, int intanceNum, int hyperperiodNum, boolean takeAllUtil, List<List<Double>> util,
			...
			List<Pair<List<DirectedAcyclicGraph>, CacheHierarchy>> allsystems) {
		...
		for (int i = 0; i < NoS; i++) {
			...
			for (DirectedAcyclicGraph d : dags) {
				d.getSchedParameters()
						.setWCET((long) Math.ceil((double) d.getSchedParameters().getPeriod() * (double) SystemParameters.utilPerTask));
	...
	public static OneSystemResults testOneCaseThreeMethod(Pair<List<DirectedAcyclicGraph>, CacheHierarchy> sys, int tasks,
		...
		SimualtorNWC cacheCASim = new SimualtorNWC(SimuType.CLOCK_LEVEL, Hardware.PROC_CACHE, Allocation.CACHE_AWARE_PREDICT_R,
				RecencyType.TIME_DEFAULT, sys.getFirst(), sys.getSecond(), cores, tableSeed, lcif);
		Pair<List<DirectedAcyclicGraph>, double[]> pair2 = cacheCASim.simulate(SystemParameters.printSim);
		
		for(DirectedAcyclicGraph d : sys.getFirst()) 
			for(Node n : d.getFlatNodes()) 
				n.sensitivity = n.getWCET();
		
		SimualtorNWC cacheCASim3 = new SimualtorNWC(SimuType.CLOCK_LEVEL, Hardware.PROC_CACHE, Allocation.CACHE_AWARE_PREDICT_R,
				RecencyType.TIME_DEFAULT, sys.getFirst(), sys.getSecond(), cores, tableSeed, lcif);
