...
//			count++;
			...
			out += d1_delay + "," + d2_delay + "," + d3_delay + "," + d1_et + "," + d2_et + "," + d3_et + "," + call1
					+ "," + call2 + "," + call3 + "\n";

//			for (int k = 0; k < d1_delays.size(); k++) {
//				out1 += d1_delays.get(k) + "," + d2_delays.get(k) + "," + d3_delays.get(k) + "," + d1_ets.get(k) + ","
//						+ d2_ets.get(k) + "," + d3_ets.get(k) + "\n";
//				if (k % 1000 == 0)
...
//					periods == null ? null : periods.get(i), false);
	...
	public static OneSystemResults testOneCaseThreeMethod(List<DirectedAcyclicGraph> dags, int tasks, int[] NoInstances,
		...
		SimualtorNWC cacheWFSim0 = new SimualtorNWC(SimuType.CLOCK_LEVEL, Hardware.PROC_CACHE,
				Allocation.CACHE_AWARE_NEW, RecencyType.TIME_DEFAULT, dags, cores, tableSeed, lcif);
		Pair<List<DirectedAcyclicGraph>, double[]> pair0 = cacheWFSim0.simulate(SystemParameters.printSim);
		...
		d3[0] = m0.stream().mapToDouble(d -> d.getDelayAndETinAvg()[0]).average().getAsDouble();
		d3[1] = m0.stream().mapToDouble(d -> d.getDelayAndETinAvg()[1]).average().getAsDouble();
		...
		noCall3.add(cacheWFSim0.noCalls);
