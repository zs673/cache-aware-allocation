...
//		case TIME_DEFAULT:
		...
		case TIME_DEFAULT:
			if (SystemParameters.v1 <= leve1Time && leve1Time <= SystemParameters.v2) {
				...
				return new Pair<Long, Integer>(
						(long) Math.ceil((double) n.getET(SystemParameters.useWCET, inRanage) * speedUp), 1);
			}
			break;
		...
		case TIME_DEFAULT:
			if (SystemParameters.v1 <= level2Time && level2Time < SystemParameters.v2) {
				...
				return new Pair<Long, Integer>(
						(long) Math.ceil((double) n.getET(SystemParameters.useWCET, inRanage) * speedUp), 2);
			}
		...
		case TIME_DEFAULT:
			...
			if (SystemParameters.v1 <= level3Time && level3Time < SystemParameters.v3) {
				...
				return new Pair<Long, Integer>(
						(long) Math.ceil((double) n.getET(SystemParameters.useWCET, inRanage) * speedUp), 3);
			}
