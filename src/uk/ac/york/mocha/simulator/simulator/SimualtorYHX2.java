...
import uk.ac.york.mocha.simulator.allocation.*;
...
public class SimualtorYHX2 {
    ...
    public SimualtorYHX2(SimuType type, Hardware hardware, Allocation alloc, RecencyType recency,
    ...
    public SimualtorYHX2(SimuType type, Hardware hardware, Allocation alloc, RecencyType recency,
        ...
        sleepingDAGs.sort((c1, c2) -> Long.compare(c1.releaseTime, c2.releaseTime)); //Rate Monotonic

        this.readyDAGs = new ArrayList<>();
        this.readyNodes = new ArrayList<>();
        this.currentExe = new Node[procNum]; //每个核当前正在运行的
        ...
        bindQueue = new ArrayList<>();
        for (int i = 0; i < procNum; i++) {
            bindQueue.add(new ArrayList<>());
        }
        deltaQueue = new Long[procNum];
        Arrays.fill(deltaQueue, Long.MIN_VALUE);
        cores = new ArrayList<>();
        for (int i = 0; i < procNum; i++) {
            cores.add(i);
        }
        ...
        this.history_level1 = new ArrayList<>();//每级缓存的分配历史
        this.history_level2 = new ArrayList<>();
        this.history_level3 = new ArrayList<>();

        this.allocHistory = new ArrayList<>();//所有核的分配历史，二维数组
    ...
    public boolean check(List<List<Node>> queue){
        boolean isLogicEmpty = true;
        for (List<Node> subList : queue) {
            if (!subList.isEmpty()) {
                isLogicEmpty = false;
                break;
            }
    ...
    public Pair<List<DirectedAcyclicGraph>, double[]> simulate(boolean printSim) {
        ...
        AllocationMethodsYHX2 allocM3 = null;
        ...
        switch (alloc) {
            ...
            case ONLINE_YHX_TEST2:
                allocM3 = new OnlineYHX_test2();
                break;
            case ONLINE_YHX_TEST:
                allocM2 = new OnlineYHX_test();
                break;
            case ONLINE_YHX_Compare:
                allocM2 = new OnlineYHX_compare();
                break;
            case ONLINE_YHX_SYN:
                allocM3 = new OnlineYHX_syn();
                break;
            default:
        ...
        while (!finished()) { //检查系统所有的workload是否完成
            /*
             ...
             */
            boolean hasAlloc = allocateAndExecute(allocM3, cacheAware, printSim);
     ...
     ******************************************************************/
    private boolean allocateAndExecute(AllocationMethodsYHX2 allocM, boolean cacheAware, boolean printSim) {
        /*
        ...
        if (alloc.toString().contains("YHX")) {
            ...
            /*
             * new: nextbusy array
             * or: add the expected start time to node attribution
             * for (int i = 0; i < cores.size(); i++) {
             * if ((localRunQueue.get(i).size() == 0 && coreTime[i] <= systemTime) ||
             * (coreTime[i] <= systemTime && localRunQueue.get(i).size() > 0 && nextbusy[i]
             * > systemTime))
             * availableCores.add(i);
             * }
             */

            if (check(bindQueue) && (readyNodes.size() == 0 || availableCores.size() == 0)) {
            } else {
                noCalls++;
                hasAlloc = allocM.allocate(dags, readyNodes, localRunQueue, bindQueue, deltaQueue, cores, coreTime, history_level1, history_level2,
        ...
        if (add) {//每次allocation allocation哪些node到哪些核 allocate了多少个
...
//        }
    ...
    public static void main(String args[]) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            // SimualtorYHX cacheBFSim = new SimualtorYHX(SimuType.CLOCK_LEVEL,
            // Hardware.PROC_CACHE,
            ...
            //
            SimualtorYHX cacheBFSim1 = new SimualtorYHX(SimuType.CLOCK_LEVEL, Hardware.PROC_CACHE, Allocation.SIMPLE,
