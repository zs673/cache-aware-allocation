...
import uk.ac.york.mocha.simulator.allocation.Onlinegyy_EFT;
...
public class SimualtorGYY {
	...
	/* The speed factor for each processor */
	List<Double> corespeed;
	...
	public SimualtorGYY(SimuType type, Hardware hardware, Allocation alloc, RecencyType recency,
			List<DirectedAcyclicGraph> dags, CacheHierarchy cache, int procNum, int recencySeed, long variation,
			boolean lcif, List<Double> corespeed) {

		this(type, hardware, alloc, recency, dags, cache, procNum, recencySeed, lcif, corespeed);
	...
	public Pair<List<DirectedAcyclicGraph>, double[]> simulate(boolean printSim) {
		...
		for (DirectedAcyclicGraph d : dags) {
			result.add(d.deepCopy());// d.deepCopy()
		}
	...
	private void allocateAndExecute(AllocationMethods allocM, boolean cacheAware, boolean printSim) {
		...
		for (int i = 0; i < currentExe.length; i++) {
			...
			// -1 or a specific value
			if (currentExe[i] == null && localReadyNodes.size() > 0 && localReadyNodes.get(0).start <= systemTime) {
			...
			for (int i = 0; i < coreTime.length; i++) {
				...
				if (tmp > systemTime)
					...
					earliestFinish = earliestFinish < localRunQueue.get(i).get(0).start ? earliestFinish
							: localRunQueue.get(i).get(0).start;//跳到繁忙核的最早完成时间还是空闲核的下一次工作开始时间
				}
			}
