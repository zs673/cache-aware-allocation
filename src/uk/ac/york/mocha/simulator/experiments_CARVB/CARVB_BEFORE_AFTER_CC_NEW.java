...
public class CARVB_BEFORE_AFTER_CC_NEW {
	...
	static double effectFactor[] = { -0.5, 0.5 };
	...
	public static void main(String args[]) {
		method = Allocation.CACHE_AWARE_NEW;
		start(nop);

		method = Allocation.CARVB;
		start(nop);
	}
	...
	public static void start(int nop) {
		...
		for (int i = 0; i < judgementLine.length; i++) {
			faults(allCores[0], judgementLine[i], allInstanceNum[0], nop, percent);
		}
	}

	public static void faults(int cores, double judgement, int instanceNum, int nop, double percent) {
			...
			runners.add(new Thread(new Runnable() {
				...
				public void run() {
					for (int i = 0; i < effectFactor.length; i++) {
						...
						String methodName = method.toString().equals("CARVB") ? "carvb" : "ajlr";

						String folderName = "result/" + "faults_new/";
						String fileName = "/before_after_" + methodName + "_" + cores + "_" + judgement + "_" + effect + "_" + id + ".txt";
	...
	public static List<ChangingNodeInfo> setUpSpecificFaults(List<DirectedAcyclicGraph> dags, double judgement, boolean fault,
		...
		int faultNodeSize = 1; // (int) Math.ceil((double) dags.get(0).getFlatNodes().size() * percent);
	...
	public static long runOne(Pair<List<DirectedAcyclicGraph>, CacheHierarchy> sys, int cores, int seed, boolean print) {
		...
		SimualtorNWC no_fault = new SimualtorNWC(SimuType.CLOCK_LEVEL, Hardware.PROC_CACHE, method, RecencyType.TIME_DEFAULT,
				sys.getFirst(), sys.getSecond(), cores, seed, true);
