...
public class CARVB_BEFORE_AFTER_NEW_ONE_NODE {
			...
			runners.add(new Thread(new Runnable() {
				...
				public void run() {
					for (int i = 0; i < effectFactor.length; i++) {
						...
						String fileName = "/before_after_" + method1Name + "_" + method2Name + "_" + cores
								+ "_" + judgement + "_" + effect + "_" + id + ".txt";
...
//		}
	...
	public static Pair<double[], double[]> getConfigurationValue(List<Node> all, Node n, double judgement) {
		...
		// note: add the feature to the pair
		configNum[0] = (double) n.getWCET();
		configNum[1] = (double) n.pathET;
		configNum[2] = (double) n.getParent().size();// in
		configNum[3] = (double) n.getChildren().size();// out
		configNum[4] = (double) (n.getParent().size() + n.getChildren().size());// in + out
	...
	public static void write(ChangingNodeInfo r, int cores, double judgement, int id, boolean append, double effect) {
		...
		String folderName = "result/" + "faults_new/";
		String fileName = "/before_after_" + method1Name + "_" + method2Name + "_" + cores + "_"
				+ judgement + "_" + effect + "_" + id + ".txt";
