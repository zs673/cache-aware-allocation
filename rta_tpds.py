...
# the following function is identical to TPDS_rta, except:
# 1. consider multi DAG interferecen
# 2. with different input variables
# this function is made for TPDS extension
# tasks should be ordered by their priorities (highest first)
# assume T=D
def TPDS_rta_multi_handler(G_list, C_list, T_list, m):
    R_list = []
    task_num = len(T_list)
    for task_idx in range(task_num):
        R_t = TPDS_rta_multi(G_list, C_list, T_list, task_idx, m)
        R_list.append(R_t)
    return R_list


def TPDS_rta_multi(G_, C_, T_list, task_idx, m):
    """ Response time analysis in:
    Qingqiang He, et. al, Intra-Task Priority Assignment in Real-Time Scheduling of DAG Tasks on Multi-cores, 2019
    """
    # --------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    G_list = ast.literal_eval(sys.argv[1])
    C_list = ast.literal_eval(sys.argv[2])
    T_list = ast.literal_eval(sys.argv[3])
    n_cores = ast.literal_eval(sys.argv[4])

    # test vectors
    # G_list = [{1:[2,3,4,5,9],2:[9],3:[6,7,8],4:[7],5:[7,8],6:[9],7:[9],8:[9],9:[]}, {1:[2,3,4,5,9],2:[9],3:[6,7,8],4:[7],5:[7,8],6:[9],7:[9],8:[9],9:[]}, {1:[2,3,4,5,9],2:[9],3:[6,7,8],4:[7],5:[7,8],6:[9],7:[9],8:[9],9:[]}]
    # C_list = [{1:4581,2:17559,3:9352,4:7826,5:8589,6:12215,7:9543,8:15078,9:11261}, {1:4581,2:17559,3:9352,4:7826,5:8589,6:12215,7:9543,8:15078,9:11261}, {1:4581,2:17559,3:9352,4:7826,5:8589,6:12215,7:9543,8:15078,9:11261}]
    # T_list = [1000000, 2000000, 3000000]
    # n_cores = 4
    ...
    R = TPDS_rta_multi_handler(G_list, C_list, T_list, n_cores)
    print(R)
