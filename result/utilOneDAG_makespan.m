...
for task = 1:9
    ...
    for m=1:methodNum
        ...
        boxplot(datam(:,InstanceIndex), 'position', (task-1)*2+m, 'widths', 0.65, 'symbol','.', 'color', colors(m,:));
        hold on;
    ...
    for m=1:methodNum
        ...
        boxplot(datam(:,instanceNo(2)+InstanceIndex), 'position', 18 + (task-1)*2+m, 'widths', 0.65, 'symbol','.', 'color', colors(m,:));
        hold on;
    end
end
xlim([0 36+1]);
...
line([18.5 18.5], [0 1.1],'LineStyle',':','color','k','LineWidth',1);
...
count = 1
for i=1.5:2:18
    xticklables(index) =  num2str((count+1) * 10);
